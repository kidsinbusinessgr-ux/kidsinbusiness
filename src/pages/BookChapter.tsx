...
const BookChapter = () => {
  ...
  const s = {
    ...
    coinsBadge: { marginTop: 16, display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.2)", borderRadius: 99, padding: "6px 16px", color: "#fff", fontWeight: 700, fontSize: 13 } as React.CSSProperties,
    ...
    phaseBtn: (active: boolean) => ({ padding: "10px 20px", borderRadius: 12, fontWeight: 700, fontSize: 14, border: "none", cursor: "pointer", background: active ? "#270F57" : "#f4eaff", color: active ? "#fff" : "#270F57", flex: 1 } as React.CSSProperties),
    ...
    primaryBtn: { display: "block", width: "100%", padding: "14px", borderRadius: 14, background: "#270F57", color: "#fff", fontWeight: 800, fontSize: 15, border: "none", cursor: "pointer", marginTop: 16 } as React.CSSProperties,
    ...
    sortItem: (cat: string) => ({ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 99, margin: "4px", fontSize: 12, fontWeight: 600, background: cat === (ch.activity.categories?.[0] || "") ? "#dbeafe" : "#fce7f3", color: cat === (ch.activity.categories?.[0] || "") ? "#1d4ed8" : "#9d174d" } as React.CSSProperties),
    ...
    doneBox: { textAlign: "center" as const, padding: "40px 20px" },
  };
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <div style={s.chNum}>Κεφάλαιο {ch.id} · {ch.emoji}</div>
        <div style={s.title}>{ch.titleEl}</div>
        <div style={s.sub}>{ch.subtitleEl}</div>
        <div style={s.coinsBadge}>🪙 {ch.coins} coins · {ch.badge} {ch.badgeName}</div>
      ...
      <div style={s.body}>
          ...
          <>
            {ch.quote && (
              <div style={{ ...s.card, background: `linear-gradient(135deg, ${grad})`, color: "#fff" }}>
                ...
                <div style={{ fontSize: 12, opacity: 0.8, fontWeight: 600 }}>— {ch.quote.author}</div>
              </div>
            )}
            <div style={s.card}>
          ...
          <>
            {quizSubmitted && (
              <div style={{ ...s.card, background: quizScore >= 60 ? "#dcfce7" : "#fee2e2", border: `2px solid ${quizScore >= 60 ? "#22c55e" : "#f87171"}` }}>
                <div style={{ fontSize: 28, textAlign: "center", marginBottom: 8 }}>{quizScore >= 80 ? "🏆" : quizScore >= 60 ? "⭐" : "💪"}</div>
                <div style={{ textAlign: "center", fontWeight: 900, fontSize: 20, color: quizScore >= 60 ? "#15803d" : "#b91c1c" }}>
                  {correctQuiz}/{ch.quiz.length} σωστά — {quizScore}%
                </div>
            ...
            <div style={s.card}>
              ...
              {ch.activity.type === "sort" && ch.activity.items && (
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#765F8F", marginTop: 12, textAlign: "center" }}>Οι κατηγορίες παρουσιάζονται ήδη ταξινομημένες.</div>
                </div>
              )}
            ...
            {!mathChecked
              ...
              : <div>
                  <div style={{ textAlign: "center", fontWeight: 900, fontSize: 18, color: "#270F57", marginBottom: 16 }}>
                    {mathCorrect}/{ch.mathChallenge.length} σωστά {mathCorrect === ch.mathChallenge.length ? "🏆" : "💪"}
                  </div>
        ...
        {phase === "done" && (
          <div style={{ ...s.card, ...s.doneBox }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: "#765F8F", marginBottom: 20 }}>{ch.badgeName}</div>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
              {ch.id < 16 && (
                <button style={{ padding: "12px 24px", borderRadius: 14, background: "#270F57", color: "#fff", fontWeight: 800, fontSize: 14, border: "none", cursor: "pointer" }}
                  onClick={() => navigate(`/book/${ch.id + 1}`)}>
                  Επόμενο Κεφάλαιο →
                </button>
              )}
              <button style={{ padding: "12px 24px", borderRadius: 14, background: "#f4eaff", color: "#270F57", fontWeight: 800, fontSize: 14, border: "none", cursor: "pointer" }}
